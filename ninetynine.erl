...
second_last([H, _|[]]) -> H;
second_last([_, H|T]) -> second_last([H|T]).

%% Get n'th element of list
nth_element(0, [H|_]) -> H;
nth_element(N, [_|T]) when N > 0 -> nth_element(N-1, T).
...
flatten([H|T], L) -> flatten(T, L ++ [H]).

%% Remove consecutive duplicate list elements
remove_duplicates([]) -> [];
remove_duplicates([H|[]]) -> [H];
remove_duplicates([H, H|T]) -> remove_duplicates([H|T]);
remove_duplicates([F, H|T]) -> [F] ++ remove_duplicates([H|T]).

%% Pack consecutive duplicate list elements into separate sublists
pack([]) -> [];
pack([H|[]]) when is_list(H) -> [H];
pack([H|[]]) -> [[H]];
pack([H, H|T]) -> pack([[H,H]|T]);
pack([[H|T], H|R]) -> pack([[H|[H|T]]|R]);
pack([[H|T], R|S]) -> [[H|T]|pack([R|S])];
pack([H, R|T]) -> [[H]|pack([R|T])].
